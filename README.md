...
generate_response = openai gpt-5.4-mini
...
**GPT-5.x on Chat Completions:** newer models expect **`max_completion_tokens`** instead of `max_tokens`. This project sends the correct field automatically when the model id looks like GPT-5 (for example `gpt-5.4-mini`, `gpt-5.2`, or snapshots containing `gpt-5`). If you still see `unsupported parameter` errors, set `OPENAI_USE_MAX_COMPLETION_TOKENS=true` to force that behavior.
