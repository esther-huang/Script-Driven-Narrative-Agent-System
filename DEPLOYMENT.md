...
```bash
python -m py_compile app/ui.py app/vector_store.py
.venv/bin/python test/test_init.py
.venv/bin/python test/test_main.py
```

Optional public landing smoke test:

```bash
PUBLIC_DEMO_MODE=true PUBLIC_DEMO_DAILY_TURN_BUDGET=9999 .venv/bin/python - <<'PY'
from streamlit.testing.v1 import AppTest
at = AppTest.from_file('main.py')
at.run(timeout=30)
assert not at.exception, at.exception
print('public demo smoke test: PASS')
PY
```
